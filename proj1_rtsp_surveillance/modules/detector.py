...
class PersonDetector:
    def __init__(self):
        ...
        self.last_person_dets = [] # Cache for skipped frames
        self.frame_count = 0

    def detect(self, frame):
        """
        Runs detection if it's time (based on SKIP_EVERY_N).
        Returns list of detections: (x1, y1, x2, y2, conf, inside_zone, cx, cy)
        """
        self.frame_count += 1
    ...
    def _run_inference(self, frame):
        """Internal method to run YOLO and process results."""
        ...
        if r.boxes is not None and len(r.boxes) > 0:
            ...
            for (x1, y1, x2, y2), c, cf in zip(boxes, clss, confs):
                ...
                self.last_person_dets.append((x1, y1, x2, y2, float(cf), inside, cx, cy))